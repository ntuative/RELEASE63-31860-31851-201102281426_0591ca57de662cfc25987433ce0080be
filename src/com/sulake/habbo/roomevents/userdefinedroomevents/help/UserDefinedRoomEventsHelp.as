package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1054:IContainerButtonWindow;
      
      private var var_1055:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_75.getXmlWindow("ude_help"));
         this.var_1637 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1055 = this.var_1637.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1055)
         {
            this.var_1055.procedure = this.onButtonClick;
         }
         this.var_1054 = this.var_1637.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1054)
         {
            this.var_1054.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_238 >= this.var_79.length - 1)
         {
            this.var_1054.disable();
         }
         else
         {
            this.var_1054.enable();
         }
         if(this.var_238 <= 0)
         {
            this.var_1055.disable();
         }
         else
         {
            this.var_1055.enable();
         }
      }
      ...
      {
         ++this.var_238;
         this.var_238 = Math.min(this.var_238,this.var_79.length - 1);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_79 = [];
         var _loc1_:int = 1;
         while(this.var_75.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_75.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_75.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_75.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_79[this.var_238][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_79[this.var_238][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_79[this.var_238][1]).value;
      ...
      {
         if(!param1 || !this.var_75 || !this.var_75.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_75.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_75.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_75.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
