package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_1960:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_871 = new Timer(this.const_1960,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_403)
         {
            ...
            this.var_403.addEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_403)
         {
            ...
            this.var_403.removeEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1331.indexOf("?") > 0)
            {
               _loc2_ = this.var_1331 + "&retry=" + this.var_870;
            }
            else
            {
               _loc2_ = this.var_1331 + "?retry=" + this.var_870;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1670 = true;
         for each(_loc2_ in this.var_1072)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2343,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1071[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1670)
         {
            this.var_1072.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_516[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_516[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_696.length)
         {
            _loc5_ = this.var_696[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_696.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_871.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_697.length > 0 && this.var_696.length < this.const_1548)
         {
            _loc1_ = this.var_697[0];
            _loc1_.startDownloading();
            this.var_696.push(this.var_697.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_697.indexOf(param1) == -1 && this.var_696.indexOf(param1) == -1)
         {
