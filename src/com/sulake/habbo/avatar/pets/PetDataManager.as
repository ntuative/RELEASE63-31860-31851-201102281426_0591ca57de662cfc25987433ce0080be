package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      {
         if(param1 == null || this.var_516 == null)
         {
            return;
         }
         var _loc2_:int = this.var_516.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_516.splice(_loc2_,1);
         }
      }
      ...
      private function notifyListeners() : void
      {
         ...
         {
            _loc1_ = this.var_516.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.HabboRoomWidgetLib_ecotronbox_card.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1456.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_635.getValue(_loc6_) == null)
               {
                  ...
                  this.var_635.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_516.indexOf(param3) == -1)
                  {
                     this.var_516.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_635.getKeys())
         {
            _loc5_ = this.var_635.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
      ...
      private function onContentLoadError(param1:Event) : void
      {
         ...
         var _loc3_:int = this.var_272.indexOf(_loc2_);
         if(_loc3_ != -1)
         {
