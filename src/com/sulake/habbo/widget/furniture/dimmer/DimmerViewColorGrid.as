package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_487.numGridItems)
         {
            return;
         }
         this.select(this.var_487.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_487 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         {
            _loc3_ = param1.buildFromXML(this.var_1754) as IWindowContainer;
            ...
            _loc3_.width = this.var_599.width;
            _loc3_.height = this.var_599.height;
            this.var_487.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_599.width,this.var_599.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_599,this.var_599.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1105.width,this.var_1105.height,true,0);
      ...
      {
         var _loc2_:int = this.var_487.getGridItemIndex(param1.target as IWindow);
