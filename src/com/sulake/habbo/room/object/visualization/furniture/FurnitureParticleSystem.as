package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_260:IRoomObjectSprite;
      ...
      private var var_1176:Number = 1.0;
      
      private var var_626:BitmapData;
      
      private var var_765:ColorTransform;
      
      private var var_1441:ColorTransform;
      
      private var var_1877:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_692:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_765 = new ColorTransform();
         this.var_765.alphaMultiplier = 1;
         this.var_1441 = new ColorTransform();
         this.var_1877 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1177)
         {
            _loc1_.dispose();
         }
         this.var_1177 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_94 = this.var_1177[param1];
         this.var_962 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1876 >= 0)
         {
            this.var_260 = this._visualization.getSprite(this.var_1876);
            if(this.var_260 && this.var_260.asset)
            {
               if(this.var_260.width <= 1 || this.var_260.height <= 1)
               {
                  return;
               }
               if(this.var_87 == null)
               {
                  this.var_87 = this.var_260.asset.clone();
                  if(this.var_765.alphaMultiplier != 1)
                  {
                     this.var_626 = new BitmapData(this.var_87.width,this.var_87.height,true,4278190080);
                  }
               }
               this.var_2050 = -this.var_260.offsetX;
               this.var_2049 = -this.var_260.offsetY;
               this.var_260.asset = this.var_87;
            }
            if(this.var_87)
            {
            ...
            if(this.var_626)
            {
               this.var_626.fillRect(this.var_626.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_94.y * this.var_1176;
         }
         return 0;
      }
      ...
      {
         if(!this.var_94 || !this.var_260)
         {
            return;
         }
         if(this.var_87 && this.var_260.asset != this.var_87)
         {
            this.var_260.asset = this.var_87;
         }
         if(this.var_962)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_962 && this.var_94.hasIgnited)
         {
            this.var_962 = true;
         }
         _loc8_ = this.var_1038 * this.var_1176;
         this.var_94.update();
         if(this.var_962)
         {
            if(this.var_94.roomObjectSpriteId >= 0)
            ...
            {
               this.var_87.draw(this.var_626,this.var_1877,this.var_765,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_94.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2050 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1176;
               _loc4_ = this.var_2049 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1176;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1441.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_87.draw(_loc7_,this._translationMatrix,this.var_1441,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2621 = parseInt(param1.@size);
         this.var_1876 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1038 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1176 = this.var_2621 / 64;
         this.var_692 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_692 = Math.min(this.var_692,1);
         this.var_765.alphaMultiplier = this.var_692;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "QuestVendingWallItemMessageComposer" ? false : true;
