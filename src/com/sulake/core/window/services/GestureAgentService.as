package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_279,this.clientWindowDestroyed);
            this.var_269 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1226 *= 0.75;
         this.var_1227 *= 0.75;
         if(Math.abs(this.var_1226) <= 1 && Math.abs(this.var_1227) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_269 != null)
         {
            this.var_269(this.var_1226,this.var_1227);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_279,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
