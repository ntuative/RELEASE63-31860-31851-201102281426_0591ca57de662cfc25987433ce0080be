package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["null"] = const_175;
         param1["bound_to_parent_rect"] = const_98;
         param1["child_window"] = const_1002;
         param1["embedded_controller"] = const_1103;
         param1["resize_to_accommodate_children"] = const_64;
         param1["input_event_processor"] = const_30;
         param1["internal_event_handling"] = const_755;
         param1["mouse_dragging_target"] = const_262;
         param1["mouse_dragging_trigger"] = const_364;
         param1["mouse_scaling_target"] = const_298;
         param1["mouse_scaling_trigger"] = const_554;
         param1["horizontal_mouse_scaling_trigger"] = const_250;
         param1["vertical_mouse_scaling_trigger"] = const_271;
         param1["observe_parent_input_events"] = const_1035;
         param1["optimize_region_to_layout_size"] = const_559;
         param1["parent_window"] = const_1021;
         param1["relative_horizontal_scale_center"] = const_174;
         param1["relative_horizontal_scale_fixed"] = const_144;
         param1["relative_horizontal_scale_move"] = const_347;
         param1["relative_horizontal_scale_strech"] = const_375;
         param1["relative_scale_center"] = const_1205;
         param1["relative_scale_fixed"] = const_661;
         param1["relative_scale_move"] = const_1010;
         param1["relative_scale_strech"] = const_1041;
         param1["relative_vertical_scale_center"] = const_193;
         param1["relative_vertical_scale_fixed"] = const_139;
         param1["relative_vertical_scale_move"] = const_401;
         param1["relative_vertical_scale_strech"] = const_315;
         param1["on_resize_align_left"] = const_620;
         param1["on_resize_align_right"] = const_439;
         param1["on_resize_align_center"] = const_520;
         param1["on_resize_align_top"] = const_885;
         param1["on_resize_align_bottom"] = const_430;
         param1["on_resize_align_middle"] = const_517;
         param1["on_accommodate_align_left"] = const_1006;
         param1["on_accommodate_align_right"] = const_521;
         param1["on_accommodate_align_center"] = const_861;
         param1["on_accommodate_align_top"] = const_1079;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_798;
         param1["route_input_events_to_parent"] = const_487;
         param1["use_parent_graphic_context"] = const_32;
         param1["draggable_with_mouse"] = const_1082;
         param1["scalable_with_mouse"] = const_1113;
         param1["reflect_horizontal_resize_to_parent"] = const_438;
         param1["reflect_vertical_resize_to_parent"] = const_426;
         param1["reflect_resize_to_parent"] = const_282;
         param1["force_clipping"] = WINDOW_PARAM_FORCE_CLIPPING;
         param1["inherit_caption"] = const_1008;
         if(param2 != null)
         {
            for(param2[param1[_loc3_]] in param1)
            {
            }
         }
      }
   }
}
