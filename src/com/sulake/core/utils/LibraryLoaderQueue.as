package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1548:int = 4;
      ...
      {
         return this.var_427.length + this.var_272.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_272.length < const_1548 && this.var_427.length > 0)
            {
               _loc1_ = this.var_427.shift();
               this.var_272.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_427.indexOf(loader);
            if(index > -1)
            {
               this.var_427.splice(index,1);
            }
            index = this.var_272.indexOf(loader);
            if(index > -1)
            {
               this.var_272.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_427)
            {
               if(param2)
               {
